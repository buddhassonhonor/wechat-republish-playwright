...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Auto-wait N seconds for you to complete Douyin login, "
            "then dump cookies (skips the interactive 'Press Enter')."
        ),
    )
...
async def capture_cookies(args: argparse.Namespace) -> int:
    ...
    async with async_playwright() as p:
        ...
        if args.login_wait_seconds is not None:
            print(f"[INFO] Auto-waiting {args.login_wait_seconds}s for login completion...")
        else:
            print(
                "[INFO] Press Enter in this terminal once the homepage shows you are logged in."
            )

        await wait_for_login_confirmation(
            page, args.url, auto_confirm_seconds=args.login_wait_seconds
        )
...
async def wait_for_login_confirmation(
    ...
    auto_confirm_seconds: Optional[int] = None,
) -> None:
