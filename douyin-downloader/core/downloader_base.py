...
class BaseDownloader(ABC):
    ...
    async def _download_aweme_assets(
        ...
        if media_type == "video":
            ...
            primary_media_files.append(video_path)
            ...
            for index, image_url in enumerate(image_urls, start=1):
                ...
                if isinstance(download_result, Path):
                    primary_media_files.append(download_result)
                else:
                    primary_media_files.append(image_path)
        ...
        elif media_type == "gallery" and primary_media_files:
            transcript_result = await self.transcript_manager.process_images(
                primary_media_files, aweme_id=aweme_id
            )
            transcript_status = transcript_result.get("status")
            if transcript_status == "skipped":
                logger.info(
                    "OCR skipped for aweme %s: %s",
                    aweme_id,
                    transcript_result.get("reason", "unknown"),
                )
            elif transcript_status == "failed":
                logger.warning(
                    "OCR failed for aweme %s: %s",
                    aweme_id,
                    transcript_result.get("error", "unknown"),
                )
