...
### Download logged-in liked posts
...
- First capture cookies with `python -m tools.cookie_fetcher --config config.yml`
- `sec_uid=self` uses the current logged-in account context, so you do not need to fill in your own `sec_uid`
- For lower request pressure, keep `rate_limit` around `0.5` to `1` and avoid high `thread` values
...
## Optional Feature: Content Text Extraction (`transcript`)

The extractor tries the following in order:

1. Prefer audio transcription first; long audio is extracted and chunked before transcription
2. If no useful text is found, sample video frames and run OCR
3. If still empty, try extracting subtitle streams
4. Image-note items are OCRed directly
...
transcript:
  ...
  local_vad_filter: true
  language_hint: ""
  audio_chunk_seconds: 150
  audio_force_chunking_above_seconds: 120
  audio_min_text_chars: 40
  ocr_enabled: true
  subtitle_enabled: true
  ocr_frame_interval_seconds: 2
  ocr_max_frames: 12
  ocr_min_text_length: 6
```

If you use an OpenAI-compatible API backend, providing the key through an environment variable is recommended:

```bash
export OPENAI_API_KEY="sk-xxxx"
```

Audio-first is the default strategy because it is cheaper and more reliable for long-form spoken content:

- extracting audio before transcription is typically lighter than processing the full video container
- long lecture/classroom videos can be chunked, which reduces the chance of getting only the opening portion
- if a long-audio result is suspiciously short, the job does not stop there and continues with OCR / subtitle fallback

Recommended backend choice:

- if you have a local GPU, prefer `backend: faster_whisper_local`
- if you do not want to run a local model, use `backend: openai_api`
- `faster-whisper` does not require `OPENAI_API_KEY`
...
### Post-process Existing Downloads

If the downloader is still running, you can extract text from already completed items without re-downloading them:

```bash
python -m tools.text_extractor -c config.yml
```

By default it reads `Downloaded/download_manifest.jsonl` and generates matching `*.transcript.txt` / `*.transcript.json` files for completed items.
To force regeneration:

```bash
python -m tools.text_extractor -c config.yml --force
```

To rebuild a specific item only, filter by `aweme_id`:
...
| `transcript.*` | Content text extraction config with audio transcription, OCR, and subtitle fallback |
...
- if `backend: openai_api`, whether `OPENAI_API_KEY` (or `transcript.api_key`) is valid
- if `backend: faster_whisper_local`, whether `faster-whisper` is installed and CUDA is usable
