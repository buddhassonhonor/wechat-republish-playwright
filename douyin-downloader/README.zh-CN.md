...
| 当前登录账号点赞列表下载 | `/user/self?showTab=like` + `mode: [like]` |
| 无水印优先 | 自动选择无水印视频源 |
| 附加资源下载 | 封面、音乐、头像、JSON 元数据 |
| 视频转写 | 可选功能，支持本地 `faster-whisper` 或 OpenAI 兼容转写接口 |
| 并发下载 | 可配置并发数，默认 5 |
...
- 先用 `python -m tools.cookie_fetcher --config config.yml` 扫码登录，保存 Cookie
- `sec_uid=self` 会使用当前登录账号上下文，不需要你手填自己的 `sec_uid`
- 建议把 `rate_limit` 设为 `0.5 ~ 1`，并尽量只开 `thread: 2~5`
...
## 可选功能：文案提取（transcript）

当前实现会按顺序尝试：

1. 优先从视频音频转文字；长音频会先抽出音轨再分段转写
2. 如果没有可用文字，再对视频抽帧做 OCR
3. 如果仍然没有，再尝试提取字幕流
4. 图文作品直接对图片做 OCR
...
如果你要走 OpenAI 兼容接口，推荐通过环境变量提供密钥：
...
- 先抽音频再转写，通常比对整段视频做处理更省资源
- 长讲座、课堂类视频更容易被分段完整覆盖，减少只拿到开头一小段的情况
- 如果长音频转写结果异常短，程序不会直接判成功，而是继续走 OCR / 字幕兜底

推荐配置：

- 有本地 GPU，优先 `backend: faster_whisper_local`
- 没有本地算力，再用 `backend: openai_api`
- `faster-whisper` 不需要 `OPENAI_API_KEY`
...
如果下载任务已经在跑，你也可以对**已完成**的作品单独做一次离线提取，不需要重新下载：

```bash
python -m tools.text_extractor -c config.yml
```

默认会读取 `Downloaded/download_manifest.jsonl`，并对已完成的作品生成对应的 `*.transcript.txt` / `*.transcript.json`。
如果你想强制重跑：
...
| `transcript.*` | 文案提取配置，支持音频转写、OCR 和字幕兜底 |
...
- 是否下载的是视频（图文不转写）
- 如果 `backend: openai_api`，检查 `OPENAI_API_KEY`（或 `transcript.api_key`）是否有效
- 如果 `backend: faster_whisper_local`，检查 `faster-whisper` 是否已安装、CUDA 是否可用
